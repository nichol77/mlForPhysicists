...
# ## Week 2: Visualizing Neural Networks with Pure Python / Tutorials
...
#
# This is essentially an extension of the week-1 notebook, but now including backpropagation.
...
#
# You may need to re-run the cell below many times to get away from the green square of gloom.
...
#
# For example, in the exrecise this week you will be asked to train networks that can do the following:
# 1.) Mimic a function that defines a circle, i.e. +1 inside the circle (easy)!
# 2.) Mimic a function that defines a circle with a hole inside (harder)!
# 3.) Mimic the following function:
#
# ```
# def my_target(y):
#     a=0.8; r=0.5; R=2.0
...
visualize_network_training(weights=[],biases=[],
    num_neurons=[2,60,30,1], # this generates randomly initialized layers of the given neuron numbers!
    bias_scale=1.0, weight_scale=1.0, # the scale of the random numbers
    target_function=my_target, # the target function to approximate
    activations=[ 'reLU',
                 'reLU',
                 'sigmoid'
    ...
    x0range=[-3,3],x1range=[-3,3],
    xspread=3,
    steps=5000, eta=2, batchsize=500,
